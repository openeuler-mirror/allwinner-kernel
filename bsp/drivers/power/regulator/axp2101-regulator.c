...
static int axp2101_regulator_probe(struct platform_device *pdev)
{
	...
	case AXP223_ID:
		...
		gpio_usb_vbus = of_property_read_bool(pdev->dev.parent->of_node,
						  "x-powers,gpio-vbus-en");
		break;
	case AXP806_ID:
	...
	if (gpio_usb_vbus) {
		switch (axp20x->variant) {
		case AXP221_ID:
			/* have to set 8F-bit4 to 1 in a40i */
			/* have to set 30F-bit7 to 1 in a40i */
			regmap_update_bits(axp20x->regmap, AXP20X_OVER_TMP,
					   AXP22X_MISC_N_VBUSEN_FUNC, AXP22X_MISC_N_VBUSEN_FUNC);

			regmap_update_bits(axp20x->regmap, AXP22X_VBUS_IPSOUT_MGMT,
					   BIT(7), BIT(7));
			break;
		default:
			break;
