...
static u32 patch_tbl_func_u02[][2] = {
	{0x00110054, 0x0018181D}, // same as jump_tbl idx 168
	{0x0011005C, 0x0018181D}, // same as jump_tbl idx 168
};
...
const struct aicbsp_firmware fw_u01[] = {
	[AICBSP_CPMODE_WORK] = {
	...
	[AICBSP_CPMODE_TEST] = {
		...
		.bt_patch      = "aic8800dc/fw_patch.bin",
		.bt_table      = "aic8800dc/fw_patch_table.bin",
		.wl_fw         = "aic8800dc/fmacfw_patch_rf.bin"
	},
};
...
static int aic8800dc_wifi_patch_config(struct priv_dev *aicdev)
{
	...
	if (aicbsp_info.cpmode == AICBSP_CPMODE_WORK) {
		...
		if (aicbsp_info.chipinfo->subrev == 0) {
			...
			jump_tbl_base = jump_tbl_u02;
			jump_tbl_size = sizeof(jump_tbl_u02)/2;
			patch_tbl_func_base = patch_tbl_func_u02;
			patch_tbl_func_num = sizeof(patch_tbl_func_u02)/sizeof(u32)/2;
...
int aicbsp_8800dc_fw_init(struct priv_dev *aicdev)
{
	...
	ret = rwnx_plat_bin_fw_upload_android(aicdev, ROM_FMAC_PATCH_ADDR, aicbsp_firmware_list[aicbsp_info.cpmode].wl_fw);
	if (ret)
		return ret;
