...
#define PLL_LOCK_CTL_OFFSET		(0x0320)
#define PLL_CPUX_OFFSET			(0)
			   ...
			   unsigned long parent_rate)
{
	...
	if (nkmp->common.features & CCU_FEATURE_CHANGE_PARENT)
		nkmp->common.change_parent(hw, SWITCH_PARENT_TO_TEMP);
	...
	if (nkmp->common.features & CCU_FEATURE_PLL_WORKAROUND) {
		reg |= ((ilog2(_nkmp.p) + 1) << nkmp->p.shift) > p_mask ?
			(ilog2(_nkmp.p) << nkmp->p.shift) & p_mask :
			((ilog2(_nkmp.p) + 1) << nkmp->p.shift) & p_mask;
	} else {
		reg |= (ilog2(_nkmp.p) << nkmp->p.shift) & p_mask;
	}
	
	/* disable cpux_pll lock */
	if (nkmp->common.features & CCU_FEATURE_PLL_LOCK_DISABLE) {
		writel(readl(nkmp->common.base + nkmp->common.reg + PLL_LOCK_CTL_OFFSET) & ~(1 << PLL_CPUX_OFFSET), nkmp->common.base + nkmp->common.reg + PLL_LOCK_CTL_OFFSET);
	}
	...
	/* enable cpux_pll lock */
	if (nkmp->common.features & CCU_FEATURE_PLL_LOCK_ENABLE) {
		writel(readl(nkmp->common.base + nkmp->common.reg + PLL_LOCK_CTL_OFFSET) | (1 << PLL_CPUX_OFFSET),nkmp->common.base + nkmp->common.reg + PLL_LOCK_CTL_OFFSET);
	}

	if (nkmp->common.features & CCU_FEATURE_CHANGE_PARENT) {
		ccu_helper_wait_for_lock_atomic(&nkmp->common, nkmp->lock);
		nkmp->common.change_parent(hw, SWITCH_PARENT_TO_NORMAL);
