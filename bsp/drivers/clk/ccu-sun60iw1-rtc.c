...
static int sun60iw1_rtc_ccu_probe(struct platform_device *pdev)
{
	struct resource *res;
	...
	return sunxi_ccu_probe(pdev->dev.of_node, reg, &sun60iw1_rtc_ccu_desc);
}

static const struct of_device_id sun60iw1_rtc_ccu_ids[] = {
	{ .compatible = "allwinner,sun60iw1-rtc-ccu" },
	{ .compatible = "allwinner,sun20iw1-rtc-ccu" },
	{ }
};

static struct platform_driver sun60iw1_rtc_ccu_driver = {
	.probe	= sun60iw1_rtc_ccu_probe,
	.driver	= {
		.name	= "sun60iw1-rtc-ccu",
		.of_match_table	= sun60iw1_rtc_ccu_ids,
	},
};

static int __init sun60iw1_rtc_ccu_init(void)
{
	int err;

	err = platform_driver_register(&sun60iw1_rtc_ccu_driver);
	if (err)
		pr_err("Fail to register sunxi_rtc_ccu as platform device\n");

	return err;
}
core_initcall(sun60iw1_rtc_ccu_init);

static void __exit sun60iw1_rtc_ccu_exit(void)
{
	platform_driver_unregister(&sun60iw1_rtc_ccu_driver);
}
module_exit(sun60iw1_rtc_ccu_exit);
