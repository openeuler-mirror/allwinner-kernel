...
#define	SUN8IW11_CPUX			0x0050

static void sun8iw11_change_parent(struct clk_hw *hw, int index)
{
	struct ccu_common *common = container_of(hw, struct ccu_common, hw);
	struct ccu_nkmp *nkmp = container_of(common, struct ccu_nkmp, common);
	void __iomem *base = nkmp->common.base;

	if (index == SWITCH_PARENT_TO_TEMP) {
		set_reg(base + SUN8IW11_CPUX, 0x1, 2, 16);
	} else if (index == SWITCH_PARENT_TO_NORMAL) {
		set_reg(base + SUN8IW11_CPUX, 0x2, 2, 16);
	}
}
...
static struct ccu_nkmp pll_cpux_clk = {
	...
	.common		= {
		...
		.hw.init	= CLK_HW_INIT("pll-cpux", "dcxo24M",
					      ...
					      CLK_SET_RATE_UNGATE |
					      CLK_IS_CRITICAL),
		.change_parent = &sun8iw11_change_parent,
//		.features	= CCU_FEATURE_CHANGE_PARENT,
		.features	= CCU_FEATURE_PLL_WORKAROUND,
	},
};
...
static const u8 const tvd_tables[] = {0, 1, 2, 3};
static struct ccu_div tvd0_clk = {
	.enable = BIT(31),
	.div	= _SUNXI_CCU_DIV(0, 4),
	.mux	= _SUNXI_CCU_MUX(24, 3),
	.mux.table = tvd_tables,
	.common	= {
		.reg		= 0x188,
