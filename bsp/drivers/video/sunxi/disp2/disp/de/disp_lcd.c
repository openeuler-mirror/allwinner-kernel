...
struct disp_lcd_private_data {
	...
	struct clk *clk_mipi_dsi[CLK_NUM_PER_DSI * DEVICE_DSI_NUM];
...
static s32 combphy_clk_config(struct clk *combphy_clk, struct disp_panel_para *panel)
{
	unsigned long combphy_rate;
	unsigned long combphy_rate_set;
	...
	combphy_rate = panel->lcd_dclk_freq * bits_per_pixel / panel->lcd_dsi_lane;

	clk_set_rate(combphy_clk, combphy_rate);
	combphy_rate_set = clk_get_rate(combphy_clk);

	if (combphy_rate != combphy_rate_set)
		DE_DBG("combphy clk was setted rate  %ld, but real rate is %ld",\
...
extern void __f_set_mipi_pll(u32 rate, u32 prate);
...
static s32 lcd_clk_config(struct disp_device *lcd)
{
	...
	if (lcdp->clk_parent) {
		...
		/* printk("pll_rate : %lu, pll_rate_set : %lu\n", pll_rate, pll_rate_set); */
	}
	...
	__f_set_mipi_pll(lcd_rate_set, pll_rate);
	...
	if (lcdp->pwm_info.dev) {
		...
		duty_ns =
		    (backlight_bright * backlight_dimming * period_ns / 256 +
		     128) / 256;
		lcdp->pwm_info.duty_ns = bright == 0 ? 0 : duty_ns;
...
#endif  /* DE_VERSION_V35X */
			lcdp->rst_bus_mipi_dsi[i] = para->rst_bus_mipi_dsi[lcd->hwdev_index];
		}
