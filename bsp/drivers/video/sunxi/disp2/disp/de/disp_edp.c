...
#if defined(SUPPORT_TCON_UPDATE_SAFE_TIME)
			 ...
			 struct disp_video_timings *p_info)
{
	...
	for (i = 0; i < EDP_POWER_NUM; i++) {
		if (i == 0)
			sprintf(sub_name, "edp_panel_power");
		else
			sprintf(sub_name, "edp_panel_power%d", i);
...
s32 edp_regulator_get_wrap(struct disp_edp_private_data *edpp)
{
	...
	for (i = 0; i < EDP_POWER_NUM; i++) {
		if (edpp->edp_panel_regulator == NULL) {
			if (strlen(edpp->edp_panel_power[i])) {
...
#if defined(CONFIG_EDP2_AW_DISP2)
