...
static s32 edid_block_valid(char *edid)
{
	...
	if (ret <= 0) {
		EDP_DBG("Error:edid header invalid\n");
		return RET_FAIL;
	}

	ret = edid_block_compute_checksum(edid);
	if (ret != 0) {
		EDP_DBG("Error:edid  checksum failed\n");
		return RET_FAIL;
	}
...
static s32 edid_block_ext_valid(char *edid)
{
	...
	if (ret <= 0) {
		EDP_DBG("Error:edid extend block is not support CEA\n");
		return RET_FAIL;
	}
...
struct edid *edp_edid_get(u32 sel)
{
	...
	if (ret < 0) {
		EDP_DBG("EDID: edp read edid block0 failed\n");
		goto fail;
	}
	...
	if (block_cnt > 1) {
		...
		if (ret < 0) {
			EDP_DBG("EDID: edp read edid extend block failed\n");
			goto fail;
		}
