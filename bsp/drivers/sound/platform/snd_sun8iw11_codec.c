...
static void snd_sunxi_dts_params_init(struct platform_device *pdev, struct sunxi_codec_dts *dts)
{
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "dac volume get failed\n");
		dts->dac_vol = 63;
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "adc gain get failed\n");
		dts->adc_gain = 3;
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "mic1 gain get failed\n");
		dts->mic1_gain = 4;
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "mic2 gain get failed\n");
		dts->mic2_gain = 4;
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "mic1 to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "mic2 to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "fmin to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "linein to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "lineinl to romix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "lineinr to lomix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "phoneout gain get failed\n");
		dts->phoneout_gain = 3;
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "hpout gain get failed\n");
		dts->hpout_gain = 63;
	} else {
					 ...
					 const char *buf, size_t count)
{
	...
	if (scanf_cnt == 1) {
		...
		while ((i < size) && (sunxi_reg_labels[i].name != NULL)) {
			output_reg_val = sunxi_regmap_read_prcm(regmap,
		...
		if (input_reg_offset > SUNXI_AUDIO_MAX_REG + size) {
			pr_err("reg offset > audio max reg[0x%x]\n", SUNXI_AUDIO_MAX_REG + size);
			return count;
		}
