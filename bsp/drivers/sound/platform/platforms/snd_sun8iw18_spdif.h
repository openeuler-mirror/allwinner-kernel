/* sound\soc\sunxi\snd_sun8iw18_spdif.h
...
#ifndef __SND_SUN8IW18_SPDIF_H
#define __SND_SUN8IW18_SPDIF_H
...
static inline int snd_sunxi_clk_init(struct platform_device *pdev, struct sunxi_spdif_clk *clk)
{
	...
	if (clk_set_parent(clk->clk_spdif, clk->clk_pll_audio)) {
				     ...
				     unsigned int freq_out)
{
	...
	if (clk_set_rate(clk->clk_spdif, freq_out)) {
