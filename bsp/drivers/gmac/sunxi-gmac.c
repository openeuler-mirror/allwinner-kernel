...
#define SUNXI_GMAC_MODULE_VERSION	"2.0.4"
...
static void fix_phy_state(struct phy_device *phydev)
{
	if(phy_read(phydev, 0x2) == 0x004d && phy_read(phydev, 0x3) == 0xd074)
	{
	}
	if(phy_read(phydev, 0x2) == 0 && phy_read(phydev, 0x3) == 282)
	{
		...
		phy_write(phydev, 0x1e, 0x27);
		phy_write(phydev, 0x1f, 0x2010);
		phy_write(phydev, 0x1e, 0xa012);
		phy_write(phydev, 0x1f, 0x68);
		phy_write(phydev, 0x1e, 0xa003);
		phy_write(phydev, 0x1f, 0xfb);
	}
	if(phy_read(phydev, 0x2) == 0x0000 && phy_read(phydev, 0x3 ) == 0x0128)
	{
		//modify YT8512 led state
		phy_write(phydev, 0x1e, 0x40c0);
		udelay(50);
		phy_write(phydev, 0x1f, 0x0c30);
		udelay(50);
		phy_write(phydev, 0x1e, 0x40c3);
		udelay(50);
		phy_write(phydev, 0x1f, 0x0330);
		udelay(50);
		//close sleep
		phy_write(phydev, 0x1e, 0x2027);
		udelay(50);
		phy_write(phydev, 0x1f, 0x2010);
		udelay(50);
	}
...
static int sunxi_gmac_resource_get(struct platform_device *pdev)
{
	...
	if (user_tx_delay >= 0 && user_tx_delay <= 7) {
		...
		netdev_info(ndev, "Info: user tx-delay: %d\n", chip->tx_delay);
	} else {
		netdev_info(ndev, "Info: dts tx-delay: %d\n", chip->tx_delay);
	}
	...
	if (user_rx_delay >= 0 && user_rx_delay <= 31) {
		...
		netdev_info(ndev, "Info: user rx-delay: %d\n", chip->rx_delay);
	} else {
		netdev_info(ndev, "Info: dts rx-delay: %d\n", chip->rx_delay);
	}
