...
static int mtdchar_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case BLKBURNBOOT0:
		...
		ret = aw_spinand_mtd_download_boot0(mtd->parent,
	...
	case BLKBURNBOOT1:
		...
		ret = aw_spinand_mtd_download_uboot(mtd->parent,
	...
	case SECBLK_READ:
		...
		ret = aw_spinand_mtd_read_secure_storage(mtd->parent,
	...
	case SECBLK_WRITE:
		...
		ret = aw_spinand_mtd_write_secure_storage(mtd->parent,
				sec_op_st.item, buf_secure, sec_op_st.len);
		if (copy_to_user(sec_op_st.buf, buf_secure, sec_op_st.len))
			ret = -EFAULT;
		kfree(buf_secure);
